...
class App1 extends React.Component {
  ...
  loadGifs = () => {
    ...
    let giphyAPI = `https://api.giphy.com/v1/gifs/search?q=${searchValue.replace(
      /\s/g,
      "+"
    )}&limit=${limit}&offset=${paginate}&api_key=ybaPDWvW02i61gblWgdFkxkrsfhsZzhi`;
    fetch(
      `https://api.giphy.com/v1/gifs/search?q=${searchValue.replace(
        /\s/g,
        "+"
      )}&limit=${limit}&offset=${paginate}&api_key=ybaPDWvW02i61gblWgdFkxkrsfhsZzhi`
    )
  ...
  loadMore = () => {
    this.setState({ paginate: this.state.paginate + 1, scrolling: true });
    this.loadGifs();
  };
  onChangeValue = (event) => {
    ...
    if (!this.state.gifs.data) {
      ...
      return (
        <div className="ui grid container">
          <input type="text" onChange={this.onChangeValue}></input>
          <InfiniteScroll
